...
---
Docker Toolbox provides a way to use Docker on older Macs that do not meet minimal system requirements for [Docker for Mac](/docker-for-mac/index.md).
...
> **Tip**: One of the advantages of the newer [Docker for Mac](/docker-for-mac/index.md) solution is that it uses native virtualization and does not require VirtualBox to run Docker.
...
2. Optionally, remove each machine. For example:
    ...
    This step is optional because if you plan to re-install Docker Machine as a part of [Docker for Mac](/docker-for-mac/index.md), you can import and continue to manage those machines through Docker.
...
6. Optionally, remove the `~/.docker` directory.
    ...
    If you want to remove Docker entirely, you can remove the `~/.docker` directory recursively. This directory stores some Docker program configuration and/or state (e.g., information about created machines such as certificates). Removing this directory is typically not necessary.
